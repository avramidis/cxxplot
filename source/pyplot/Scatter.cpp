...
namespace cxxplot {
    ...
    template<class inputType>
    Scatter<inputType>::Scatter(std::vector<inputType>& x, std::vector<inputType>& y, Kwargs& args)
    {
        this->figure();
        initialize();
        draw(x, y, args);
    ...
    template<class inputType>
    void
    Scatter<inputType>::draw(std::vector<inputType>& x, std::vector<inputType>& y, Kwargs& args)
    {
        PyObject* x_py = ConvertToNumpy::vector_to_numpy(x);
        PyObject* y_py = ConvertToNumpy::vector_to_numpy(y);
        ...
        PythonCalls::pyobject_call_with_checks(scatter, scatter_args, args.get_pydict());
    }
