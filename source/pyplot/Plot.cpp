...
namespace cxxplot {
    ...
    CXXPLOT_EXPORT Plot<inputType>::Plot(bool create_new_figure) {
    ...
    template <class inputType>
    CXXPLOT_EXPORT Plot<inputType>::Plot(std::vector<inputType> &y,
    ...
    template <class inputType>
    CXXPLOT_EXPORT Plot<inputType>::Plot(std::vector<inputType> &x,
    ...
    Plot<inputType>::Plot(std::vector<inputType> &x, std::vector<inputType> &y,
                          std::string fmt, bool create_new_figure) {
        if (create_new_figure) {
            this->figure();
        }
        initialize();
    ...
    template <class inputType>
    CXXPLOT_EXPORT Plot<inputType>::~Plot() {
        Py_DECREF(plot);
    }

    template <class inputType>
    void Plot<inputType>::initialize_x_default(size_t x_size) {
        x_default.resize(x_size);
        std::iota(x_default.begin(), x_default.end(), inputType(0));
    }

    template <class inputType>
    void Plot<inputType>::initialize() {
        plot = PythonCalls::get_pyobject_function_by_string(
            this->matplotlib_pyplot, "plot");
    }

    template <class inputType>
    void Plot<inputType>::draw(std::vector<inputType> &x,
                               std::vector<inputType> &y) {
    ...
    template <class inputType>
    void Plot<inputType>::draw(std::vector<inputType> &x,
                               std::vector<inputType> &y, Kwargs &kwargs) {
        PyObject *args = generate_args_pytuple(x, y);
    ...
    template <class inputType>
    void Plot<inputType>::add_data(std::vector<inputType> &x,
                                   std::vector<inputType> &y) {
        draw(x, y);
    }
    ...
    template CXXPLOT_EXPORT class Plot<int>;

    template CXXPLOT_EXPORT class Plot<float>;

    template CXXPLOT_EXPORT class Plot<double>;
} // namespace cxxplot